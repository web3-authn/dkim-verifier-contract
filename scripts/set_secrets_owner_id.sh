...
: "${SECRETS_OWNER_ID:?Set SECRETS_OWNER_ID in .env to the Outlayer secrets owner account ID}"
...
JSON_ARGS="$(jq -n --arg secrets_owner_id "${SECRETS_OWNER_ID}" '{secrets_owner_id: $secrets_owner_id}')"

echo "Setting secrets owner ID on ${CONTRACT_ID} to ${SECRETS_OWNER_ID}"
