...
: "${SECRETS_PROJECT_ID:?Set SECRETS_PROJECT_ID in .env (e.g. w3a-v1.near/tatchi-xyz-email-recovery)}"
...
JSON_ARGS="$(jq -n --arg secrets_project_id "${SECRETS_PROJECT_ID}" '{secrets_project_id: $secrets_project_id}')"

echo "Setting secrets project ID on ${CONTRACT_ID} to ${SECRETS_PROJECT_ID}"

near contract call-function as-transaction "$CONTRACT_ID" set_secrets_project_id \
  json-args "$JSON_ARGS" \
...
near contract call-function as-read-only "$CONTRACT_ID" get_secrets_project_id \
  json-args '{}' \
  network-config "$NEAR_NETWORK_ID"
